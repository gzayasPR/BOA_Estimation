...
int main(int argc, char** argv){
	...
	if(argc != 8){
	  fprintf(stderr,"USAGE:: %s <win-size> <nr-founders> <SNP-file> <CEU-haps> <YRI-haps> <ADMX-geno> <out-file>\n", argv[0]);
	  exit(0);
	}
	...
	fprintf(stderr,"Read %d anc0 %d anc1 training haplotypes\n",anc_map[0].nr_hap,anc_map[1].nr_hap);
...
void read_pos_file(const char* filename){
  ...
  snps_pos = (int*)malloc( (nr_total_snps)*sizeof(int));
  //snps_map = (double*)calloc(nr_total_snps+1, sizeof(double));
  rewind(fin);	
  i=0;
  int tempint;
