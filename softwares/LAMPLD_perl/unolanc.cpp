...
void read_hmm_from_file (FILE *f, HAP_HMM *hmm , int h );
...
int main(int argc, char** argv){
  ...
  if(argc < 8){
    //simple interface;
    fprintf(stderr,"USAGE:: %s <win-size> <nr-founders> <SNP-file> <CEU-haps> <YRI-haps> <NA-haps> <ADMX-geno> <out-file> <trainedhmm>\n", argv[0]);
    exit(0);
  }	
  nr_snp = atol(argv[1]);
  ...
  read_samples(argv[7]);
  //read_true_sol("haps.anc"); // true solution??
  //read_map_file("/home/bp45/projects/localanc/latino-trio/latino/chr1.genmap"); //use recombination rate file???
  FILE *fhmm = NULL;
  if(argc==10){
    fhmm = fopen(argv[9],"r");
    fprintf(stderr,"reading trained HMM from file\n");
    if(!fhmm){
      fprintf(stderr,"cannot open hmm infile!\n");
      exit(1);
    }
  }
  ...
  //allocate memory; to add a memcheck in the next iteration 
  ...
  for(int idx_win = 0; idx_win < (nr_total_snps/nr_snp); idx_win++){
	...
	read_hmm_from_file (fhmm, &anc_map[h].hmm , h );
      }else{
	trainHMM(h,start_pos);
      }
    }
    //fprintf(stderr,"training done\n");
    //compute the logprob of given admixed genotype under every pair of ancestries
  ...
  /*fill in arrays for Viterbi computation*/	
  for(int idx_win = 0; idx_win < (nr_total_snps/nr_snp); idx_win++){
    if(idx_win == 0){
      for(int igen = 0; igen < nr_admx_samples; igen++){
	for(int i =0; i < nr_anc; i++){
	...
	for(int i =0; i < nr_anc; i++){
	  for(int j = i ; j < nr_anc; j++){
		...
		double trans = 1;	
		if(prev_i != i ){
		  //trans = recomb_rate*(snps_pos[i_snp-(nr_snp/2)+ii]-snps_pos[i_snp-(nr_snp/2) + ii-1]));
  ...
  /*perform Viterbi using the backpointers*/
  for(int igen = 0; igen < nr_admx_samples; igen++){
    double max = -99999;
  ...
  */
  ...
  for(int i_snp = nr_snp; i_snp < (nr_total_snps-nr_snp); i_snp+=nr_snp){
    ...
    //train HMMS that overlap breakpoint position
    for(int h = 0; h < hmmCount; h++){
	...
	//find best position to place the breakpoint using forward/backward computations over the two inferred pair of ancestries
	...
	//maximize SNP positon; do not let SNP be on the edge
...
char* getgeno( char* hap1, char* hap2, int nr_snp){
	char *gen =  (char*)calloc (nr_total_snps+10,sizeof(char));
	for(int i =0; i < nr_snp; i++){
...
void read_hmm_from_file (FILE *f, HAP_HMM *hmm , int h ){
  ...
  for(int i =0; i < nr_snp; i++){
    for(int j =0; j < nr_founders; j++){
      ...
      (*hmm).states[i][j].prob_em[0] = f1;
      (*hmm).states[i][j].prob_em[1] = 1-f1;
      if(j==(nr_founders-1)){
        fscanf(f,"\n");
      }else{
        fscanf(f," ");
