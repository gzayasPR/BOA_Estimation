...
STATE**	states_temp;
#define MAXWIN 1000
void remove_short_chunks(int);
double getavganc(int snp,int anc);
void printanc();
int main(int argc, char** argv){
	...
	if(argc != 10){
	  fprintf(stderr,"USAGE:: %s <win-size> <nr-founders> <SNP-file> <CEU-haps> <YRI-haps> <NA-haps> <ADMX-hap> <out-file> <smoothing0-no,1yes>\n", argv[0]);
	  exit(0);
	}
	...
	trans_temp = (double***)malloc((MAXWIN-1)*sizeof(double**));
	...
	states_temp = (STATE**)malloc(MAXWIN*sizeof(STATE*));
	...
	for(int h = 0; h < hmmCount; h++){
	  anc_map[h].forward_hap_array = (double**)malloc(MAXWIN*sizeof(double*));
	  for(int i =0; i < MAXWIN; i++){
	    anc_map[h].forward_hap_array[i] = (double*)malloc(nr_founders*sizeof(double));
	  }	
	  anc_map[h].backward_hap_array = (double**)malloc(MAXWIN*sizeof(double*));
	  for(int i =0; i < MAXWIN; i++)    {
	...
	for(int idx_win = 0; idx_win < (nr_total_snps/nr_snp); idx_win++){
	  ...
	  for(int igen = 0; igen < nr_admx_samples; igen++){
	    for(int i =0; i < nr_anc; i++){
	      unoProb[idx_win][igen][i]  = log(gethapprob(admx_samples[igen], anc_map[i].hmm, start_pos));
		  //log(totalProbUno_forward_fast(admx_samples[igen], anc_map[i].hmm,anc_map[i].hmm,start_pos));
	...
	for(int idx_win = 0; idx_win < (nr_total_snps/nr_snp); idx_win++){
	  if(idx_win == 0){
	    for(int igen = 0; igen < nr_admx_samples; igen++){
	      for(int i =0; i < nr_anc; i++){
		  forwardUnoProb[idx_win][igen][i] = unoProb[idx_win][igen][i];
		  //fprintf(stderr,"%d %d (-1) ->(%d) %f\n",igen,idx_win,i,forwardUnoProb[idx_win][igen][i]);						
	      }
	    }
	  }else{
	    for(int igen = 0; igen < nr_admx_samples; igen++){
		  ...
		  for(int prev_i =0; prev_i < nr_anc; prev_i++){
		      double trans = 1;	
		      if(prev_i != i ){
			trans = recomb_rate*(snps_pos[idx_win*nr_snp] - snps_pos[(idx_win)*nr_snp-1]);
		      }
		      double prob  = log(trans) + (forwardUnoProb[idx_win-1][igen][prev_i])
			+ (unoProb[idx_win][igen][i]);
		      if(prob > maxp){
			maxp = prob;
			max_i = prev_i;
		      }
		  ...
		  forwardUnoProb[idx_win][igen][i] = maxp;
		  backPointerI[idx_win][igen][i] = max_i;
		  //fprintf(stderr,"%d %d (%d) ->(%d) %f\n",igen,idx_win,max_i,i,forwardUnoProb[idx_win][igen][i]);						
	      }
	    }
	  }
	}
	...
	for(int igen = 0; igen < nr_admx_samples; igen++){
	  ...
	  int cur_i = 0;
	  for(int i =0; i < nr_anc; i++){
	    if(forwardUnoProb[idx_win][igen][i] > max){
	      max = forwardUnoProb[idx_win][igen][i];
	      cur_i = i;
	    }
	  }
		
	  fprintf(stderr,"found max at %f (%d)\n",max,cur_i);
	  while(idx_win >= 0){
	    //fprintf(stderr,"%d (%d)\n",idx_win,cur_i);
	    
	    for(int i = idx_win*nr_snp; i<(idx_win+1)*nr_snp && i< nr_total_snps ;i++){
	      admx_inf_array[igen][i][cur_i]++;
	    }
	    ...
	    int tcur_i = backPointerI[idx_win][igen][cur_i];
	    //fprintf(stderr,"%d %d\n",tcur_i,tcur_j);
	    cur_i = tcur_i;
	    idx_win--;
	...
	printanc();
	remove_short_chunks(2000000);
	printanc();

	if(smoothing){
	  int ambgPos=0;
	  double forwardArray[2][MAXWIN];
	  double backwardArray[2][MAXWIN];
	  int anc[2];
	  //nr_snp = 00;
	  ...
	  for(int i_snp = nr_snp; i_snp < (nr_total_snps-nr_snp); i_snp+=nr_snp){
		...
		//   fprintf(stderr,"%d %d (%d -> %d)\n",i_hap,i_snp,admx_samples_inf[i_hap][i_snp-1],admx_samples_inf[i_hap][i_snp]);
		/*found a breakpoint -> readjusting it*/
		anc[0] = (int)(admx_samples_inf[i_hap][i_snp-1]);
		anc[1] = (int)(admx_samples_inf[i_hap][i_snp]);
		// fprintf(stderr,"%xd %d\n",anc[0],anc[1]);                                                                          
		//getchar();                                                                                                        
		for(int i_anc = 0 ; i_anc < 2; i_anc++){
		  fill_forward_backward_hap_arrays(admx_samples[i_hap], anc_map[ anc[i_anc] ].hmm,  i_snp-(nr_snp/2));
		...
		for(int ii = 10; ii < nr_snp-10; ii++){
		  ...
		  if( forwardArray[0][ii]+backwardArray[1][ii] +rprob > best_prob){
		    best_i = ii;
		    best_prob = forwardArray[0][ii]+backwardArray[1][ii]+rprob;
		  }
		}
		/*new breakpoint location*/
		fprintf(stderr,"%d global (%d %d) (%d -> %d), prob (%f)\n",i_hap,anc[0],anc[1], i_snp, i_snp-(nr_snp/2)+best_i,best_prob);
		if(best_i < (nr_snp/2)){
		  for(int ii = i_snp-(nr_snp/2)+best_i;ii<i_snp;ii++){
		    admx_samples_inf[i_hap][ii] = anc[1];
		}
		}else{
		  for(int ii = i_snp; ii< i_snp-(nr_snp/2)+best_i; ii++){
		    admx_samples_inf[i_hap][ii] = anc[0];
		  }
		}
	      }
	    }
 ...
 for(int i =0; i < nr_admx_samples;i++){
   ...
   for(int j =1; j < nr_total_snps;j++){
     if(admx_samples_inf[i][j] != admx_samples_inf[i][j-1]){
       fprintf(stderr," %d:%d",j,admx_samples_inf[i][j-1]);
     }
   }
   fprintf(stderr," %d:%d\n",nr_total_snps,admx_samples_inf[i][nr_total_snps-1]);
 }
}

void remove_short_chunks(int length){
  fprintf(stderr,"removing chunks smaller than %d SNPs\n",length);
  for(int i =0; i < nr_admx_samples;i++){
    for(int j =1; j < nr_total_snps;j++){
      if(admx_samples_inf[i][j] != admx_samples_inf[i][j-1]){
	if( snps_pos[j] < length){
	  fprintf(stderr," removing %d (%d %d )\n",i,0,j);
	  //telomere
	  for(int jj =0; jj < j-1; jj++){
	    admx_samples_inf[i][jj] = admx_samples_inf[i][j];
	  }
	}
	if((snps_pos[nr_total_snps-1] - snps_pos[j]) < length){
	  fprintf(stderr," removing %d (%d %d )\n",i,j,nr_total_snps);
	  //telomere
	  for(int jj =j; jj < nr_total_snps; jj++){
	    admx_samples_inf[i][jj] = admx_samples_inf[i][j-1];
	  }
	}
	int jj=j+1;
	while(admx_samples_inf[i][jj] == admx_samples_inf[i][jj-1] && jj<nr_total_snps) 
	  jj++;
	if(jj < nr_total_snps && admx_samples[i][jj] == admx_samples[i][j-1]){
	  if( (snps_pos[jj-1] -snps_pos[j]) < length){
	    fprintf(stderr," removing %d (%d %d)\n",i,j,jj);
...
void fix_ancestry_hap(int right_snp){
  for(int i =0; i < nr_admx_samples;i++){
    for(int j =0; j < right_snp;j++){
      double max = 0;
      int a1=0;
      int a2=0;
      for(int ii =0; ii < nr_anc;ii++){
	  if(admx_inf_array[i][j][ii] > max){
	    a1=ii;
	    max = admx_inf_array[i][j][ii];
	  }
      }
      admx_samples_inf[i][j] = (a1);     
    }
  }
}
...
void alloc_mem_other()
{
	forward_hap_array = (double**)malloc(MAXWIN*sizeof(double*));
	for(int i =0; i < MAXWIN; i++)
	{
		forward_hap_array[i] = (double*)malloc(nr_founders*sizeof(double));
	}	
	backward_hap_array = (double**)malloc(MAXWIN*sizeof(double*));
	for(int i =0; i < MAXWIN; i++)
	{
	...
	totalProbUnoMatrix_forward = (double***)malloc(MAXWIN*sizeof(double**)); 
	totalProbUnoMatrix_backward = (double***)malloc(MAXWIN*sizeof(double**));
	totalProbUnoMatrix_unk = (double**)malloc(MAXWIN*sizeof(double*));
...
{	
	hmm->states = (STATE**)malloc(MAXWIN*sizeof(STATE*));
	for(int i =0; i < MAXWIN; i++)
	{
	...
	hmm->trans = (double***)malloc((MAXWIN-1)*sizeof(double**));
	for(int i =0; i < MAXWIN-1; i++)
	{
